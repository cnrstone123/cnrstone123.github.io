...
app00.controller("SelectCtrler", function ($scope, $http) {
	...
	$scope.LstSGG = $scope.LstSGGALL[0];
	$scope.LstLawUMD = $scope.LstLawUMDALL[0];
	$scope.LstGovUMD = $scope.LstGovUMDALL[0];

	$scope.cdSIDO = $scope.LstSIDO[0]; $scope.cdSGG = $scope.LstSGG[0];
	$scope.cdLawUMD = $scope.LstLawUMD[0]; $scope.cdGovUMD = $scope.LstGovUMD[0];
	...
	$scope.changedSIDO = function () { // event-handler of cdSGG-select-click
		...
		$scope.LstLawUMD = $scope.LstLawUMDALL[0]; $scope.LstGovUMD = $scope.LstGovUMDALL[0];
		...
		worker.onmessage = function (event) {
			...
			for (var row = 0; row < results.length; row++) {
				results[row].values.map(function (item) {
					vals = item[0].split(',');
					var codeValue = vals[0].substr(5, 3);
					var mingValue = vals[1].replace(orgnm, '');
					rows.push('{"code":"' + codeValue + '", "ming":"' + mingValue + '__' + codeValue + '"}');
				});
			}
	...
	var url = "http://apis.data.go.kr/B552015/NpsBplcInfoInqireService",
	...
	var oper1 = function(){
		var compName  = (document.getElementById('compName').value||''),
			...
			paramStr  = "ldong_addr_mgpl_dg_cd=" + (cdSIDO.value||'')
				+"&ldong_addr_mgpl_sggu_cd=" + (cdSGG.value||'')
				+"&ldong_addr_mgpl_sggu_emd_cd=" + (cdLawUMD.value||'')
				+"&wkpl_nm=" + (compName||'')
				+"&bzowr_rgst_no=" + (compRegNo||'')
				+"&pageNo=" + 10
				+"&numOfRows=" + 1 + keyStr;
		if (compName.trim().length == 0 && compRegNo.trim().length == 0){
			alert('both company-name and regno is empty.');
			return;
		}
		if (compRegNo.trim().length > 0 && compRegNo.trim().length < 6){
			alert('it has to be at least 6-digits.');
			return;
		}
